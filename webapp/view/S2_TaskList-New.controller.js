...
sap.ui.define([
	...
	TaskListCustomAttributeHelper, DataManager, BaseController, Application, Main, PositiveAction, NegativeAction, Button, BaseLog, ConfirmationDialogManager,
	ForwardPopUp, ResubmitPopUp, MultiSelectDialog, ActionHelper, CommonFunctions, ForwardSimplePopUp, Conversions, syncStyleClass,
	Device, MessagePopoverItem, library, MessagePopover, Fragment, DateFormat, jQuery, Constants, Version
) {
	"use strict";
	...
	BaseController.extend("cross.fnd.fiori.inbox.view.S2_TaskList", {
		...
		onInit: function() {
			...
			var oComponent = Application.getImpl().getComponent();
			...
			var autoPopinAvailable = Version(sap.ui.version).compareTo(Constants.AUTO_POPIN_MODE_MIN_VERSION) >= 0;
			if (autoPopinAvailable) {
		...
		_initPersonalization: function() {
			if (!sap.ushell || !sap.ushell.Container || !sap.ushell.Container.getServiceAsync) {
				this.getView().getModel("taskListView").setProperty("/personalizationActive", false);
				return;
			}
			...
			sap.ushell.Container.getServiceAsync("Personalization").then(function (oPersonalizationService) {
				var oPersonalizer = oPersonalizationService.getPersonalizer({
					...
					item: "taskListTable" // Maximum of 40 characters applies to this key as well !! Not found reference for this restriction
				});
				...
				this.getView().getModel("taskListView").setProperty("/personalizationActive", !!oPersonalizationService);
			}.bind(this));
		},
		...
		*/},
		...
		showDecisionDialog: function(oDecisionOption) {
			var bShowNote = typeof oDecisionOption.CommentSupported === "boolean" ? oDecisionOption.CommentSupported : true;
			...
			this._oConfirmationDialogManager.showDecisionDialog({
				...
				noteMandatory: bShowNote && oDecisionOption.CommentMandatory,
